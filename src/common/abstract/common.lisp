       ...
       ,@(iterate
           ...
           (collect `(defun ,fname
                       ...
                       (declare (optimize (speed 3) (safety 0))
                                (type ,name argument))
                       (etypecase argument
                         (,tagged-name (,(struct-accessor-name tagged-name slot) argument))
                         (,untagged-name (,(struct-accessor-name untagged-name slot) argument)))))
           (collect `(defun (setf ,fname)
                         (new-value argument)
                       (declare (optimize (speed 3))
                                (type ,name argument))
                       (etypecase argument
                         (,tagged-name (setf (,(struct-accessor-name tagged-name slot) argument)
                                             new-value))
                         (,untagged-name (setf (,(struct-accessor-name untagged-name slot) argument)
                                               new-value))))))
