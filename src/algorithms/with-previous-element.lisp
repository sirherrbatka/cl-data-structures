         ...
         ((element)
           (if previous-bound
               (let ((*previous-element* previous-element))
                 (setf previous-element element previous-bound t)
...
(defgeneric with-previous-element (range)
  (:generic-function-class with-previous-element-function)
  (:method (range)
    (apply-range-function range #'with-previous-element nil)))


(defmethod apply-layer ((range cl-ds:traversable)
                        (fn with-previous-element-function)
                        all)
  (make-proxy range 'forward-with-previous-element-proxy))
...
(defmethod apply-layer ((range fundamental-bidirectional-range)
                        (fn with-previous-element-function)
                        all)
  (make-proxy range 'bidirectional-with-previous-element-proxy))


(defmethod apply-layer ((range fundamental-random-access-range)
                        (fn with-previous-element-function)
                        all)
  (make-proxy range 'random-access-with-previous-element-proxy))
