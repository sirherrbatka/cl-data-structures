...
(defun approximated-counts-distance (a-sketch b-sketch)
  ...
  (flet ((counter-mean (counter line)
           (declare (optimize (speed 3) (safety 0))
                    (type (simple-array non-negative-fixnum (* *)) counter)
                    (type fixnum line))
           (iterate
             (declare (type fixnum i length total))
             (with total = 0)
             (with length = (array-dimension counter 1))
             (for i from 0 below length)
             (incf total (aref counter line i))
             (finally (return (coerce (/ total length)
                                      'single-float))))))
    (iterate
      (declare (type (simple-array non-negative-fixnum (* *))
